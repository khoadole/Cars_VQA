...
def create_BOW(corpus:str, max_features=384, oov_token="<unk>") -> CountVectorizer:
    '''Create a BOW model from the corpus'''
    # Add OOV token to the corpus
    corpus_with_oov = corpus + [oov_token]
    vectorizer = CountVectorizer(max_features=max_features, binary=True, token_pattern=r"(?u)\b\w+\b|<unk>")
    vectorizer.fit(corpus_with_oov)
    return vectorizer

def vectorize_question_to_bow(vectorizer:CountVectorizer, question:str, oov_token="<unk>") -> np.ndarray:
...
def create_global_dictionaries(train_df, val_df, test_df):
    ...
    global_questions_bow = create_BOW(all_questions)
...
def create_local_dictionaries(train_df):
    questions_bow = create_BOW(train_df['question'])
...
def load_question_bow(config:configparser.ConfigParser) -> CountVectorizer:
...
# if __name__ == "__main__":
    ...
    # vec = vectorize_question_to_bow(vectorizer, "What is the color of this car fuck brand, and car name ?")
    # print("vector:", vec)
